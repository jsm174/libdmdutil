...
Pixelcade::Pixelcade(struct sp_port* pSerialPort, int width, int height)
{
   ...
   for (int i = 0; i < PIXELCADE_MAX_QUEUE_FRAMES; ++i)
      m_framePool.push((uint8_t*)malloc(m_length * 3 / 2));
   m_pThread = nullptr;
   m_running = false;
...
Pixelcade::~Pixelcade()
{
...

   {
      ...
      while (!m_frames.empty()) {
         uint8_t* pFrame = m_frames.front();
         m_frames.pop();
         free(pFrame);
      }

      while (!m_framePool.empty()) {
         uint8_t* pFrame = m_framePool.front();
         m_framePool.pop();
         free(pFrame);
      }
   }
...
void Pixelcade::Update(uint16_t* pData)
{
   ...
   if (!m_framePool.empty()) {
      ...
      pFrame = (uint8_t*)malloc(m_length * 3 / 2);

   FrameUtil::SplitIntoRgbPlanes(pData, m_length, 128, 16, pFrame);
      ...
      while (m_running) {
         ...
         if (m_frames.size() > PIXELCADE_MAX_QUEUE_FRAMES) {
            ...
            while (!m_frames.empty()) {
               ...
               enum sp_return response = sp_blocking_write(m_pSerialPort, pFrame, m_length * 3 / 2, PIXELCADE_COMMAND_WRITE_TIMEOUT);
